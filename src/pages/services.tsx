...
export default function ServicesPage() {
  ...
  const services: Service[] = [
    {
      name: 'Coffee',
      description: 'Savor our premium Arabica and Robusta blends, freshly brewed for a perfect cup every time.',
      image: './Services/coffee.jpg', // Placeholder image path
    },
    {
      name: 'Fresh Drinks',
      description: 'Enjoy refreshing smoothies, iced teas, and cold-pressed juices made with natural ingredients.',
      image: './Services/drink.jpg', // Placeholder image path
    },
    {
      name: 'Healthy Food',
      description: 'Indulge in nutritious salads, wraps, and bowls packed with fresh, wholesome ingredients.',
      image: './Services/healthyy.jpg', // Placeholder image path
    },
    {
      name: 'Breakfast',
      description: 'Start your day with hearty options like croissants, oatmeal, and breakfast sandwiches.',
  ...
  const whyChooseUsBenefits = [
    {
      ...
      description: 'We use only the finest, locally sourced ingredients for every dish.',
      icon: (
    ...
    },
    {
      ...
      description: 'Tailor your order to your taste with our flexible menu choices.',
      icon: (
        <svg className="w-8 h-8 text-green-600" fill="none" stroke="currentColor" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
  ...
  return (
    <div className="pt-45 p-6 bg-green-50  min-h-screen">
      <div className="max-w-5xl mx-auto">
        <h1 className={`text-3xl font-bold text-center mb-10 text-amber-800 ${isMounted ? 'animate-fade' : ''}`}>
          Our Services
        </h1>
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-2 gap-8">
          {services.map((service, index) => (
            <ServiceCard key={service.name} name={service.name} description={service.description} image={service.image} index={index} />
          ))}
        </div>
        ...
        <div className="text-center mt-10">
          <Link href="/product" className="inline-block px-6 py-3 bg-amber-900 text-white rounded hover:bg-green-700 hover:animate-bounce transition-colors duration-300">
            Explore Our Menu
          </Link>
        </div>
      </div>
