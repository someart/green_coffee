...
export function Chatbot() {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    const response =
      recommendations[lowerCaseFlavor] || recommendations.default;

    setMessages([
      ...messages,
      { sender: 'User  ', text: flavor },
  ...
  return (
    <div className="fixed bottom-5 right-5 z-50">
      ...
      {isOpen ? (
        <div
          ref={chatbotRef}
          className="bg-white p-6 rounded-lg border border-gray-200 shadow-lg w-[440px] h-[634px]"
        >
          ...
          <div className="pr-4 h-[474px] overflow-y-auto">
            {messages.map((msg, index) => (
              <div
                key={index}
                className={`flex gap-3 my-4 text-gray-600 text-sm ${msg.sender === 'Green' ? 'flex-1' : ''}`}
              >
                ...
                <div
                  className={`bg-${msg.sender === 'User   ' ? 'blue' : 'green'}-100 p-2 rounded-lg`}
                >
                  {msg.text}
                </div>
              </div>
          ...
          <form onSubmit={handleSubmit} className="flex mt-4">
            ...
            <button
              type="submit"
              className="bg-amber-900 text-white rounded-lg px-4 ml-2"
            >
              Send
            </button>
          </form>
        ...
        <button
          ...
          className="bg-amber-900 text-white rounded-full p-5 shadow-lg"
        >
          Open Chat
        </button>
      )}
